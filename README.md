...
1. **标签打样实测**——用真标签机打 3 张 12mm 标签,分别用低端安卓机和旧 iPhone 扫。二维码是标准 QR version 1(21×21),12mm 标签实际可打印宽度只有 9~10mm,每模块约 0.43mm,已接近打印机极限。实测结果可能需要反过来调整编号长度或纠错等级(见 `backend/app/routers/assets.py` 的 `_make_qr`)。
...
另外 `segno.make()` 必须带 `micro=False`。编号只有 7 个字符,segno 默认会挑 **Micro QR**(15×15),而 ZXing 和浏览器 `BarcodeDetector` **都不支持 Micro QR**——标签打出来,我们自己的扫码页反而读不出来。
