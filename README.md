...
### 标签用 Code 128-C 一维码,不是二维码

换码制的理由是**打印精度**,不是编码能力。

12mm 标签可打印宽度约 9~10mm,QR 要把 29 个模块(21×21 加四周静默区)塞进去,
每个模块 0.31mm —— 而 203dpi 打印机一个点是 0.125mm,算下来 **2.5 个点**。
没法均匀分配,有的模块占 2 点、有的占 3 点,边缘参差,解码时好时坏。这就是
实际打样中扫不出来的原因。
...
| | 每条/模块宽度 | 是打印点的整数倍 | 尺寸 |
|---|---|---|---|
| 旧方案 QR | 0.31mm | ✗ 2.5 点 | 9mm 见方 |
| **Code 128-C** | **0.375mm** | **✓ 3 点** | **33 × 8mm** |

**条宽比原来还宽 20%,而且是整数倍。**

**为什么另存一个 6 位数字而不是直接编 asset_tag**:C 子集一个符号字符编两位
数字,密度是 B 子集的两倍。`PC-0001` 走 B 子集要 30mm 且随前缀长短变化
(`LIGHT-0001` 要 38.5mm),纯数字固定 22mm —— 省下的宽度换成更粗的条。

人看到的仍然是 `asset_tag`:标签上条码底下印的是 `PC-0001`,手输的也是它。
那串数字纯粹给机器。`by-tag` 接口两种都认。

条码号取主键补零,天然唯一,不用再维护一个计数器;必须是**偶数位**,奇数位会
逼 Code 128 从 C 子集切出去,白白多占宽度。

**只出 SVG 不出 PNG**:位图在打印时会被重采样,条宽又变回不是打印点的整数倍,
绕回原来的坑。矢量图交给打印机自己栅格化才能保证每根条落在整点上。

代价是一维码**没有纠错**(QR 在 ECC H 下能容 30% 破损),被划一道就废。但当前
的问题是印不准而不是印坏了,而且标签上一直印着可人工识别的资产编号兜底。

### 二维码的三个参数都不能随便动(旧标签仍在用时参考)
